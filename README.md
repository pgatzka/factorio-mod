...
- The mod loads in the current stable headless Factorio with the base game alone, and its scripts run without errors. A small harness mod (`.github/ci/harness`) builds a roboport with debris around it and fails the check when the powered roboport marks nothing or the unpowered one marks something.
